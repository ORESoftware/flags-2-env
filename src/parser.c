...
static int f2e_tty_forced(const char *name, int detected) {
  ...
  if (f2e_streq(value, "1") || f2e_streq(value, "true")) {
    return 1;
  }
  if (f2e_streq(value, "0") || f2e_streq(value, "false")) {
    return 0;
  }
  return detected;
}
...
/* Mirrors terminal_context.c: any of these names present and non-empty. */
...
static int f2e_tty_dumb(void) {
  ...
  return term && f2e_streq(term, "dumb");
}
  ...
  while (fgets(line, sizeof(line), file)) {
    ...
    *eq = '\0';
    ...
    for (size_t i = 0; i < seen->count; i++) {
      ...
      if (f2e_streq(seen->entries[i].file, relative_path)) {
        f2e_audit_add(audit, 0,
                      "%s:%d: %s was already assigned at line %d; the later value wins",
                      relative_path, line_number, key, seen->entries[i].line);
      } else {
        f2e_audit_add(audit, 0,
                      "%s:%d: %s is also set in %s:%d; %s wins because it is read later",
                      relative_path, line_number, key, seen->entries[i].file,
                      seen->entries[i].line, relative_path);
      }
      break;
    }

    if (seen->count < F2E_MAX_ENV_FILE_KEYS) {
      f2e_strlcpy(seen->entries[seen->count].key, key,
                  sizeof(seen->entries[0].key));
      f2e_strlcpy(seen->entries[seen->count].file, relative_path,
                  sizeof(seen->entries[0].file));
      seen->entries[seen->count].line = line_number;
      seen->count++;
    }
