...
import { apply, auditConfig, auditConfigStatus, auditEnv, auditEnvStatus, coerce, CoercionError, completionScript, generateTypes, helpTable, helpTableForArgv, parse, parseOverridesFromArgs, parseStructured, resolveCommands, parseFromArgs } from "./lib.mjs";
...
assert.throws(
  ...
  (error) => error instanceof CoercionError && error.errors.some((message) => message.includes("flags.ratio")),
);
